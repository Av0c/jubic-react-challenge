...
import { PhoneState, BirthdayState, FormState, HomeState, FormDataState, PhoneProps } from "./types";
...
class Phone extends React.Component<PhoneProps, PhoneState> {
    ...
    render() {
        ...
        let phoneRender = [(<div key="phone-prefix" className="phone-digit prefix">+358</div>)];
        for (let i = 0; i < maxPhoneLength; i++) {
            let currentNumber = phoneNumber + currentPhoneDigit;
            currentNumber = currentNumber.substring(i, i+1);
            if (!hasStarted) { // Show empty boxes
                phoneRender.push(
                    <div key={"key"+i} className="phone-digit active"></div>
                )
            } else {
                phoneRender.push(
...
class Birthday extends React.Component<any, BirthdayState> {
    constructor(props: {}) {
        super(props);
        this.state = {
            birthdayMonthRender: [],
    ...
    componentDidMount() {
        let birthdayMonthOptions = ["April", "August", "December", "February", "January", "July", "June", "March", "May", "November", "October", "September"];

        let birthdayMonthRender = [<option key="placeholder" disabled value="">-- Select month --</option>];
        for (let i = 0; i < birthdayMonthOptions.length; i++) {
        ...
        let birthdayDateRender = [<option key="placeholder" disabled value="">-- Select date --</option>];
        for (let i = 0; i < birthdayDateOptions.length; i++) {
...
class Form extends React.Component<any, FormState> {
    ...
    changeHandler(e: React.ChangeEvent<HTMLInputElement>) {
        ...
        this.setState({
            [name]: value,
        } as Pick<FormState, keyof FormState>); // https://github.com/DefinitelyTyped/DefinitelyTyped/issues/26635
    }
...
class Home extends React.Component<any, HomeState> {
    ...
    componentDidMount() {
        let forms: FormDataState[] = ls.get("forms");
        // console.log(forms);
        if (!forms) {
    ...
    formAdd(formData: FormDataState) {
    ...
    closePopup(e: any) { // Only to please the compiler
        let id = e.target.id;
        console.log(id);
    ...
    formsRender() {
        ...
        if (forms.length == 0) {
            formsRender.push(
                <tr key="tr-empty-warning" className="test">
                    <td key="td-empty-warning" colSpan={3} className="empty-text-warn"><i>Empty table (Add a form to get started).</i></td>
                </tr>
            );
        }
        for (let i = 0; i < forms.length; i++) {
            let form: FormDataState = forms[i];
            let f = i;
            formsRender.push(
                <tr key={"table-row-"+i}>
                    <td>{form.name}</td><td>{form.desc}</td><td>
            ...
            <div id="popup-overlay" className="popup-overlay" onClick={(e) => {this.closePopup(e)}}>
                <div className="popup-container">
                    <div className="popup-header">
                        Details
                        <div className="popup-close-button" onClick={(e) => {this.closePopup(e)}}><i id="popup-close-button"  className="material-icons">&#xe14c;</i></div>
                    </div>
        ...
        return (
            <div>
                <div className="center-container">
                    ...
                    <Form
                        formAdd={(state: FormDataState) => {this.formAdd(state)}}
                    />
